...
if (isset($_GET["action"]))
   {
	...
	if($row['Active'] == '0' )
	{
		...
		//http://localhost/Employee_Switch_Persons/login.php?action=logout&vars=0
...
function SaveEmpDetail($pk,$name,$loginname,$dept,$desig)
{
	global $con;
	//echo "commingdfdsfs";
	 	$sql = "UPDATE  `employee` SET  
...
{
	global $con;
	 $sql ="SELECT `PK`,`Employee_Name`,`loginID`,`Cost_Department`,`Designation` FROM `employee` WHERE `PK` = $pk";
...
function saveEditProjects($projNo,$hod,$tl,$about,$stdate,$deadDate)
{
	global $con;
...
/*	
	$temp  = $stdate; //"2013-Oct-10";
	$stdate = date("y-m-d", strtotime($temp));
	$temp  = $deadDate; //"2013-Oct-10";
	$deadDate = date("yy-mm-dd", strtotime($temp));
	*/
...
`TeamLead` =  '$tl',
`StartDate` =  '$stdate',
`DeadLIneDate` =  '$deadDate' WHERE  `projects`.`PK` =$projNo";
	//echo $sql;
	$result = mysql_query($sql,$con) or die(mysql_error());
}
...
{
	global $con;
	$sql = "SELECT `HOD`,`TeamLead`,`StartDate`, `DeadLIneDate`,`About` FROM `projects` where `PK` = $projID"; // get all project name
	$result = mysql_query($sql,$con) or die(mysql_error());
...
function ProjectsPreDays($projID, $dateRng)
{
	...
	$no_result = mysql_num_rows($result); // total rows 
	 ...
	 echo "<b>ProjectName</b>,<b>Modeler Name<b>,<b>Time<b>,<b>Half,<b>date</b>";
    ...
    function EmpProjNames($id)		// when or if email id already exist return 1
   {
      ...
      //echo "here is a function id = $id\n";
		...
		else	// show all name 
		{
			...
			//echo "not modeler: ";
		$sql = "SELECT `PK`,`Employee_Name` , `Designation` FROM `employee` "; // get all member name 
        // echo  $sql;
        $result = mysql_query($sql,$con) or die(mysql_error());
...
function whiteboardfn()
{
	...
	else
	{
		...
		$sql = "SELECT `PK`,`Name` FROM `projects`,`emp_curr_proj`  where (`projects`.`Status` = 1 or `projects`.`Status` = 0 ) and (`emp_curr_proj`.`ETPK` =  $userID and `emp_curr_proj`.`Active` =  1)";
      ...
      if($no_result != 0) //mean user still login no Senior or other logout this member
		{
			for ($i=0;$i<=$no_result-1;$i++)
            {
                echo "<tr>";
			...
			global $abscentEmp;
			 ...
			 echo "<td class=".$no_result.">sfsdf</td>";
...
function GetEmpNameReturn($despk) // return name
    {
        $con = mysql_connect("localhost","root","");
        $db=mysql_select_db("ttl_employee_switch", $con);
